/**
 * @epicai/chariot — Test Harness HTTP Backend
...
export class HttpHarnessBackend implements HarnessBackend {
  ...
  async start(): Promise<void> {
    ...
    this.port = await new Promise<number>((resolve, reject) => {
      ...
      if (!cp) throw new Error('invariant: childProcess not spawned');
      const cpStdout = cp.stdout;
      if (!cpStdout) throw new Error('invariant: childProcess stdout is null');
      cpStdout.on('data', (data: Buffer) => {
