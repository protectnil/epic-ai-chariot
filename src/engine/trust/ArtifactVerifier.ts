/**
 * @epicai/chariot — Artifact Verifier
...
export class ArtifactVerifier {
  ...
  async verify(
    ...
    if (cosignBundle && this.config.enforceSLSA) {
      slsaLevel = this.checkSLSA(cosignBundle, actualDigest);
      if (!slsaLevel) {
        const result: VerificationResult = {
          valid: false,
          artifactDigest: actualDigest,
  ...
  private async verifySigstore(content: Buffer, bundleB64: string): Promise<string> {
    try {
      ...
      const sigstoreModule = await import('sigstore') as { verify: (bundle: SigstoreBundle, content: Buffer) => Promise<void> };
      const bundleJson = Buffer.from(bundleB64, 'base64').toString('utf-8');
      const bundle = parseJsonObject<SigstoreBundle>(bundleJson) ?? {};

      await sigstoreModule.verify(bundle, content);
      ...
      throw new Error(`Sigstore verify: ${err instanceof Error ? err.message : String(err)}`, { cause: err });
    }
  }
  ...
  private checkSLSA(bundleB64: string, expectedDigest: string): 'SLSA_L1' | 'SLSA_L2' | 'SLSA_L3' | undefined {
    try {
      const bundleJson = Buffer.from(bundleB64, 'base64').toString('utf-8');
      const bundle = parseJsonObject<SigstoreBundle>(bundleJson);
      ...
      const payload = parseJsonObject<SLSAStatement>(Buffer.from(payloadB64, 'base64').toString('utf-8'));
      ...
      const digestMatch = subjects.some(s => {
