/**
 * @epicai/chariot — AWS Secrets Manager Provider
...
export class AWSSecretsManager implements SecretsProvider {
  ...
  async getSecretForAdapter(adapterName: string, key: string): Promise<string> {
    ...
    const awsMod = await import('@aws-sdk/client-secrets-manager') as Record<string, new (...args: unknown[]) => unknown>;
    const GetSecretValueCommand = awsMod['GetSecretValueCommand'];
  ...
  private async getClient(): Promise<unknown> {
    ...
    // @ts-expect-error — @aws-sdk/client-secrets-manager is an optional peer dependency
    const awsMod2 = await import('@aws-sdk/client-secrets-manager') as Record<string, new (...args: unknown[]) => unknown>;
    const SecretsManagerClient = awsMod2['SecretsManagerClient'];
