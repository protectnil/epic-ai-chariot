...
export class OperaPmsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OperaPmsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Oracle Opera PMS (OHIP): configuration object is required');
    }
    for (const __k of (['baseUrl', 'clientId', 'clientSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Oracle Opera PMS (OHIP): ' + __k + ' is required');
      }
    }
