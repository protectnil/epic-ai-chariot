...
export class DiscourseMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DiscourseConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('discourse-latest: configuration object is required');
    }
    for (const __k of (['baseUrl', 'apiKey', 'apiUsername'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('discourse-latest: ' + __k + ' is required');
      }
    }
