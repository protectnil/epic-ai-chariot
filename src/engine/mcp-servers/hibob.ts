...
export class HiBobMCPServer extends MCPAdapterBase {
  ...
  constructor(config: HiBobConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('HiBob: configuration object is required');
    }
    for (const __k of (['serviceUserId', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('HiBob: ' + __k + ' is required');
      }
    }
