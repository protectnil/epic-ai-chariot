...
export class MXMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MXConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('MX Financial Data: configuration object is required');
    }
    for (const __k of (['clientId', 'apiKey'] as Array<keyof MXConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('MX Financial Data: ' + __k + ' is required');
      }
    }
