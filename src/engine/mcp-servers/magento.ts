...
export class MagentoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MagentoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Magento: configuration object is required');
    }
    for (const __k of (['accessToken', 'baseUrl'] as Array<keyof MagentoConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Magento: ' + __k + ' is required');
      }
    }
