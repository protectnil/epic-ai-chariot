...
export class RelativityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: RelativityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Relativity: configuration object is required');
    }
    for (const __k of (['accessToken', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Relativity: ' + __k + ' is required');
      }
    }
