...
export class AWSS3MCPServer extends MCPAdapterBase {
  ...
  constructor(config: AWSS3Config) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('AWS S3: configuration object is required');
    }
    for (const __k of (['accessKeyId', 'secretAccessKey', 'region'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('AWS S3: ' + __k + ' is required');
      }
    }
  ...
  private generatePresignedUrl(args: Record<string, unknown>): ToolResult {
    if (!args.bucket || !args.key) return { content: [{ type: 'text', text: 'bucket and key are required' }], isError: true };
