...
export class IntelProductCatalogueMCPServer extends MCPAdapterBase {
  ...
  constructor(config: IntelProductCatalogueConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Intel Product Catalogue: configuration object is required');
    }
    for (const __k of (['username', 'password', 'clientId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Intel Product Catalogue: ' + __k + ' is required');
      }
    }
