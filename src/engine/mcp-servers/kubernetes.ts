...
export class KubernetesMCPServer extends MCPAdapterBase {
  ...
  constructor(config: KubernetesConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Kubernetes: configuration object is required');
    }
    for (const __k of (['api_server', 'token'] as Array<keyof KubernetesConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Kubernetes: ' + __k + ' is required');
      }
    }
