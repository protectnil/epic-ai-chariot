...
export class Dynamics365MCPServer extends MCPAdapterBase {
  ...
  constructor(config: Dynamics365Config) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('dynamics-365: configuration object is required');
    }
    for (const __k of (['accessToken', 'organizationUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('dynamics-365: ' + __k + ' is required');
      }
    }
