...
export class PlaidInvestmentsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PlaidInvestmentsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Plaid Investments: configuration object is required');
    }
    for (const __k of (['clientId', 'secret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Plaid Investments: ' + __k + ' is required');
      }
    }
