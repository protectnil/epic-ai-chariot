...
export class FhirMCPServer extends MCPAdapterBase {
  ...
  constructor(config: FhirConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('fhir: configuration object is required');
    }
    for (const __k of (['baseUrl', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('fhir: ' + __k + ' is required');
      }
    }
