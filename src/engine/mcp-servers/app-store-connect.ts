...
export class AppStoreConnectMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AppStoreConnectConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('App Store Connect: configuration object is required');
    }
    for (const __k of (['keyId', 'issuerId', 'privateKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('App Store Connect: ' + __k + ' is required');
      }
    }
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            name: {
              type: 'string',
              description: 'Device name (e.g. "My iPhone 16 Pro")',
            },
            udid: {
              type: 'string',
