...
export class MonteCarloMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MonteCarloConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Monte Carlo: configuration object is required');
    }
    for (const __k of (['mcdId', 'mcdToken'] as Array<keyof MonteCarloConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Monte Carlo: ' + __k + ' is required');
      }
    }
