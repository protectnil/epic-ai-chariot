...
export class AWSCloudWatchMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AWSCloudWatchConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('AWS CloudWatch: configuration object is required');
    }
    for (const __k of (['accessKeyId', 'secretAccessKey', 'region'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('AWS CloudWatch: ' + __k + ' is required');
      }
    }
  ...
  private async getMetricData(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    let queries: unknown[];
    try { queries = JSON.parse(args.metric_data_queries as string) as unknown[]; } catch { return { content: [{ type: 'text', text: 'metric_data_queries must be valid JSON' }], isError: true }; }
  ...
  private async getMetricStatistics(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (args.dimensions) {
      ...
      try { dims = JSON.parse(args.dimensions as string) as { Name: string; Value: string }[]; } catch { return { content: [{ type: 'text', text: 'dimensions must be valid JSON' }], isError: true }; }
      dims.forEach((d, i) => {
