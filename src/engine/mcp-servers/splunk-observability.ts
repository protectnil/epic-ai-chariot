...
export class SplunkObservabilityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SplunkObservabilityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Splunk Observability: configuration object is required');
    }
    for (const __k of (['accessToken', 'realm'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Splunk Observability: ' + __k + ' is required');
      }
    }
