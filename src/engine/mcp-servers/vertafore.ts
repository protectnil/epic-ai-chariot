...
export class VertaforeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VertaforeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Vertafore: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'agencyId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Vertafore: ' + __k + ' is required');
      }
    }
