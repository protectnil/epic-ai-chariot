...
export class BulkSMSMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BulkSMSConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('BulkSMS: configuration object is required');
    }
    for (const __k of (['credentials'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('BulkSMS: ' + __k + ' is required');
      }
    }
