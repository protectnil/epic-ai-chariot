...
export class NetskopeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: NetskopeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Netskope: configuration object is required');
    }
    for (const __k of (['apiToken', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Netskope: ' + __k + ' is required');
      }
    }
