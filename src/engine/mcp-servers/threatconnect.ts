...
export class ThreatConnectMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ThreatConnectConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ThreatConnect: configuration object is required');
    }
    for (const __k of (['accessId', 'secretKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('ThreatConnect: ' + __k + ' is required');
      }
    }
