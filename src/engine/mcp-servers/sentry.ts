...
export class SentryMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SentryConfig) {
    super();
    if (config === null) { throw new Error('SentryMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Sentry: configuration object is required');
    }
    for (const __k of (['authToken', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sentry: ' + __k + ' is required');
      }
    }
