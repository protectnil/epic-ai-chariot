...
export class BandwidthMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BandwidthConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Bandwidth: configuration object is required');
    }
    for (const __k of (['accountId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Bandwidth: ' + __k + ' is required');
      }
    }
