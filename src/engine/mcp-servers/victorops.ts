...
export class VictorOpsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VictorOpsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('VictorOps: configuration object is required');
    }
    for (const __k of (['apiId', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('VictorOps: ' + __k + ' is required');
      }
    }
