...
export class BlackboardMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BlackboardConfig) {
    super();
    if (config === null) { throw new Error('BlackboardMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Blackboard Learn: configuration object is required');
    }
    for (const __k of (['applicationKey', 'applicationSecret', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Blackboard Learn: ' + __k + ' is required');
      }
    }
