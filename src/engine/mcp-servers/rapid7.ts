...
export class Rapid7MCPServer extends MCPAdapterBase {
  ...
  constructor(config: Rapid7Config) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Rapid7 InsightVM: configuration object is required');
    }
    for (const __k of (['host', 'username', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Rapid7 InsightVM: ' + __k + ' is required');
      }
    }
