...
export class TelegramMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TelegramConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Telegram: configuration object is required');
    }
    for (const __k of (['botToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Telegram: ' + __k + ' is required');
      }
    }
