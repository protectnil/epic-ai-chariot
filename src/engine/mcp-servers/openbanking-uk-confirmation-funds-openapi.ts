...
export class OpenBankingUKConfirmationFundsOpenapiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenBankingUKConfirmationFundsOpenapiConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('OpenBanking UK Confirmation of Funds: configuration object is required');
    }
    for (const __k of (['accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('OpenBanking UK Confirmation of Funds: ' + __k + ' is required');
      }
    }
