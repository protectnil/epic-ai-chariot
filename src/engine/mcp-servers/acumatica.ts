...
export class AcumaticaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AcumaticaConfig) {
    super();
    if (config === null) { throw new Error('AcumaticaMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Acumatica: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'username', 'password', 'instanceUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Acumatica: ' + __k + ' is required');
      }
    }
