...
export class HarnessCICDMCPServer extends MCPAdapterBase {
  ...
  constructor(config: HarnessCICDConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Harness CI/CD: configuration object is required');
    }
    for (const __k of (['apiKey', 'accountId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Harness CI/CD: ' + __k + ' is required');
      }
    }
