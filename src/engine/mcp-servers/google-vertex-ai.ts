...
export class GoogleVertexAIMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VertexAIConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Vertex AI: configuration object is required');
    }
    for (const __k of (['projectId', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Vertex AI: ' + __k + ' is required');
      }
    }
