...
export class MailboxvalidatorValidationMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MailboxValidatorConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('MailboxValidator Email Validation: configuration object is required');
    }
    if (config.apiKey === undefined || config.apiKey === null || config.apiKey === '') {
      throw new Error('MailboxValidator Email Validation: apiKey is required');
    }
