...
export class PostmarkAppAccountMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PostmarkAppAccountConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Postmark Account API: configuration object is required');
    }
    for (const __k of (['accountToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Postmark Account API: ' + __k + ' is required');
      }
    }
