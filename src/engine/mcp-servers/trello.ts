...
export class TrelloMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TrelloConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Trello: configuration object is required');
    }
    for (const __k of (['apiKey', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Trello: ' + __k + ' is required');
      }
    }
