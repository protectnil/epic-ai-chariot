...
export class ConfluenceMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ConfluenceConfig) {
    super();
    if (config === null) { throw new Error('ConfluenceMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Atlassian Confluence: configuration object is required');
    }
    for (const __k of (['instance', 'email', 'apiToken', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Atlassian Confluence: ' + __k + ' is required');
      }
    }
