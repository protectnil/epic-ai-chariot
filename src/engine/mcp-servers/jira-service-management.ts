...
export class JiraServiceManagementMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JiraServiceManagementConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Jira Service Management: configuration object is required');
    }
    for (const __k of (['domain', 'email', 'apiToken'] as Array<keyof JiraServiceManagementConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Jira Service Management: ' + __k + ' is required');
      }
    }
