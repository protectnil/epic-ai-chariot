...
export class NbgGrMCPServer extends MCPAdapterBase {
  ...
  constructor(config: NbgGrConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('NBG Greece Open Banking: configuration object is required');
    }
    for (const __k of (['accessToken', 'clientId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('NBG Greece Open Banking: ' + __k + ' is required');
      }
    }
