...
export class PendoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PendoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Pendo: configuration object is required');
    }
    for (const __k of (['integrationKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Pendo: ' + __k + ' is required');
      }
    }
