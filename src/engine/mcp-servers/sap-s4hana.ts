...
export class SAPS4HANAMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SAPS4HANAConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('SAP S/4HANA: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'subdomain', 'region'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('SAP S/4HANA: ' + __k + ' is required');
      }
    }
