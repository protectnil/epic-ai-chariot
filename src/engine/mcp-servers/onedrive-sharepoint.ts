...
export class OneDriveSharePointMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OneDriveSharePointConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('OneDrive & SharePoint: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'tenantId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('OneDrive & SharePoint: ' + __k + ' is required');
      }
    }
