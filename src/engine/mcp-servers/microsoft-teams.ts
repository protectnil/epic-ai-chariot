...
export class MicrosoftTeamsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MicrosoftTeamsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Microsoft Teams: configuration object is required');
    }
    if (config.accessToken === undefined || config.accessToken === null || config.accessToken === '') {
      throw new Error('Microsoft Teams: accessToken is required');
    }
