...
export class StellastraMCPServer extends MCPAdapterBase {
  ...
  constructor(config: StellastraConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Stellastra: configuration object is required');
    }
    for (const __k of (['authEmail', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Stellastra: ' + __k + ' is required');
      }
    }
