...
export class SecureframeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SecureframeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Secureframe: configuration object is required');
    }
    for (const __k of (['apiKey', 'apiSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Secureframe: ' + __k + ' is required');
      }
    }
  ...
  static catalog() {
    return {
      ...
      toolNames: [
        ...
        'list_user_accounts', 'list_tprm_vendors', 'list_repositories',
        'list_integration_connections', 'list_repository_framework_scopes'
      ],
  ...
  get tools(): ToolDefinition[] {
    return [
      ...
      },
      {
        ...
        description: 'List user accounts in the Secureframe platform with optional filters for status and role',
        inputSchema: {
          type: 'object',
          properties: {
            status: {
              type: 'string',
              description: 'Filter by account status: active, inactive, suspended (optional)',
            },
            role: {
              type: 'string',
              description: 'Filter by user role: admin, auditor, user (optional)',
            },
            page: {
              type: 'number',
              description: 'Page number for pagination (default: 1)',
      ...
      {
        name: 'list_tprm_vendors',
        description: 'List vendors in the Third-Party Risk Management program with optional filters for risk tier and status',
        inputSchema: {
          type: 'object',
          properties: {
            risk_tier: {
      ...
      },
      {
        ...
        description: 'List code repositories with compliance status and optional filters for framework and status',
        inputSchema: {
          type: 'object',
          properties: {
            framework: {
              type: 'string',
              description: 'Filter by compliance framework: soc2, iso27001, hipaa, gdpr (optional)',
            },
            status: {
              type: 'string',
              description: 'Filter by repository status: compliant, non_compliant, pending (optional)',
            },
            page: {
              type: 'number',
              description: 'Page number for pagination (default: 1)',
      ...
      },
      {
        ...
        description: 'List all integration connections with their sync status and optional filters for status',
        inputSchema: {
          type: 'object',
          properties: {
            status: {
              type: 'string',
              description: 'Filter by connection status: active, inactive, error (optional)',
            },
            page: {
              type: 'number',
              description: 'Page number for pagination (default: 1)',
      ...
      {
        name: 'list_repository_framework_scopes',
        description: 'List framework scopes for repositories with optional filters for framework and repository',
        inputSchema: {
          type: 'object',
          properties: {
            framework: {
    ...
    try {
      switch (name) {
        ...
        case 'list_user_accounts':
          return this.listUserAccounts(args);
        case 'list_tprm_vendors':
          return this.listTprmVendors(args);
        case 'list_repositories':
          return this.listRepositories(args);
        case 'list_integration_connections':
          return this.listIntegrationConnections(args);
        case 'list_repository_framework_scopes':
          return this.listRepositoryFrameworkScopes(args);
        default:
  ...
  private async listUserAccounts(args: Record<string, unknown>): Promise<ToolResult> {
    const params: Record<string, string> = {
      page: String((args.page as number) || 1),
      per_page: String((args.per_page as number) || 25),
    };
    if (args.status) params['filter[status]'] = args.status as string;
    if (args.role) params['filter[role]'] = args.role as string;
    return this.get('/v1/user_accounts', params);
  }

  private async listTprmVendors(args: Record<string, unknown>): Promise<ToolResult> {
    const params: Record<string, string> = { page: String((args.page as number) || 1) };
    if (args.risk_tier) params['filter[risk_tier]'] = args.risk_tier as string;
    if (args.status) params['filter[status]'] = args.status as string;
    return this.get('/v1/tprm_vendors', params);
  }
  ...
  private async listRepositories(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (args.framework) params['filter[framework]'] = args.framework as string;
    if (args.status) params['filter[status]'] = args.status as string;
    return this.get('/v1/repositories', params);
  }

  private async listIntegrationConnections(args: Record<string, unknown>): Promise<ToolResult> {
    const params: Record<string, string> = { page: String((args.page as number) || 1) };
    if (args.status) params['filter[status]'] = args.status as string;
    return this.get('/v1/integration_connections', params);
  }

  private async listRepositoryFrameworkScopes(args: Record<string, unknown>): Promise<ToolResult> {
    const params: Record<string, string> = {
      page: String((args.page as number) || 1),
      per_page: String((args.per_page as number) || 25),
    };
    if (args.framework) params['filter[framework]'] = args.framework as string;
    if (args.repository_id) params['filter[repository_id]'] = args.repository_id as string;
    return this.get('/v1/repository_framework_scopes', params);
  }
}
