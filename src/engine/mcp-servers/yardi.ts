...
export class YardiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: YardiConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Yardi: configuration object is required');
    }
    for (const __k of (['baseUrl', 'username', 'password', 'serverName', 'database', 'platform'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Yardi: ' + __k + ' is required');
      }
    }
