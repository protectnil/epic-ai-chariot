...
export class GainsightMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GainsightConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Gainsight: configuration object is required');
    }
    for (const __k of (['accessKey', 'domainUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Gainsight: ' + __k + ' is required');
      }
    }
