...
export class MattermostMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MattermostConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mattermost: configuration object is required');
    }
    for (const __k of (['token', 'baseUrl'] as Array<keyof MattermostConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Mattermost: ' + __k + ' is required');
      }
    }
