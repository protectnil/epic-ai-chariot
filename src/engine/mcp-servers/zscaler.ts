...
export class ZscalerMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ZscalerConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Zscaler: configuration object is required');
    }
    for (const __k of (['baseUrl', 'username', 'password', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Zscaler: ' + __k + ' is required');
      }
    }
