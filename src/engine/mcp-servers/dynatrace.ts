...
export class DynatraceMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DynatraceConfig) {
    super();
    if (config === null) { throw new Error('DynatraceMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('dynatrace: configuration object is required');
    }
    for (const __k of (['apiToken', 'baseUrl', 'platformUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('dynatrace: ' + __k + ' is required');
      }
    }
