...
export class OpenBankingProjectChMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenBankingProjectChConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Open Banking Project Switzerland (Swiss NextGen API): configuration object is required');
    }
    for (const __k of (['bearerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Open Banking Project Switzerland (Swiss NextGen API): ' + __k + ' is required');
      }
    }
