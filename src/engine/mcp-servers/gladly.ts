...
export class GladlyMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GladlyConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Gladly: configuration object is required');
    }
    for (const __k of (['organization', 'email', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Gladly: ' + __k + ' is required');
      }
    }
