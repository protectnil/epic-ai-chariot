...
export class QRadarMCPServer extends MCPAdapterBase {
  ...
  constructor(config: QRadarConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('IBM QRadar: configuration object is required');
    }
    for (const __k of (['host', 'secToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('IBM QRadar: ' + __k + ' is required');
      }
    }
