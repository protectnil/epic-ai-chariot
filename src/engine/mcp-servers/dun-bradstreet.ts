...
export class DunBradstreetMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DunBradstreetConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('dun-bradstreet: configuration object is required');
    }
    for (const __k of (['consumerKey', 'consumerSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('dun-bradstreet: ' + __k + ' is required');
      }
    }
