...
export class ClioMCPServer extends MCPAdapterBase {
  ...
  private async fetchJSON(url: string, options: RequestInit = {}): Promise<ToolResult> {
    ...
    const normalizedUrl = pathPart.endsWith('.json')
      ? url
      : `${pathPart}.json${queryPart ? `?${queryPart}` : ''}`;
