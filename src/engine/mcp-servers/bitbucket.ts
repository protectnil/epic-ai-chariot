...
export class BitbucketMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BitbucketConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Bitbucket: configuration object is required');
    }
    for (const __k of (['username', 'appPassword'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Bitbucket: ' + __k + ' is required');
      }
    }
