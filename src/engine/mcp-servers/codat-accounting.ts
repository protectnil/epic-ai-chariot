...
export class CodatAccountingMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CodatAccountingConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Codat Accounting: configuration object is required');
    }
    for (const __k of (['apiKey', 'companyId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Codat Accounting: ' + __k + ' is required');
      }
    }
