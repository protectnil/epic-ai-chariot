...
export class ZoomConnectMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ZoomConnectConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ZoomConnect: configuration object is required');
    }
    for (const __k of (['email', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('ZoomConnect: ' + __k + ' is required');
      }
    }
