...
export class MapboxMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MapboxConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mapbox: configuration object is required');
    }
    if (config.accessToken === undefined || config.accessToken === null || config.accessToken === '') {
      throw new Error('Mapbox: accessToken is required');
    }
  ...
  private getStaticMap(args: Record<string, unknown>): ToolResult {
