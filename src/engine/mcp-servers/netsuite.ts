...
export class NetSuiteMCPServer extends MCPAdapterBase {
  ...
  constructor(config: NetSuiteConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Oracle NetSuite: configuration object is required');
    }
    for (const __k of (['accountId', 'consumerKey', 'consumerSecret', 'tokenId', 'tokenSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Oracle NetSuite: ' + __k + ' is required');
      }
    }
