...
export class MixpanelMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MixpanelConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mixpanel: configuration object is required');
    }
    for (const __k of (['projectToken', 'serviceAccountUsername', 'serviceAccountSecret', 'projectId'] as Array<keyof MixpanelConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Mixpanel: ' + __k + ' is required');
      }
    }
