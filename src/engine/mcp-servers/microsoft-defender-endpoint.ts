...
export class MicrosoftDefenderEndpointMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MicrosoftDefenderEndpointConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Microsoft Defender for Endpoint: configuration object is required');
    }
    for (const __k of (['tenantId', 'clientId', 'clientSecret'] as Array<keyof MicrosoftDefenderEndpointConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Microsoft Defender for Endpoint: ' + __k + ' is required');
      }
    }
  ...
  private async apiPost(url: string, headers: Record<string, string>, body: unknown): Promise<ToolResult> {
    ...
    const data: unknown = text ? JSON.parse(text) as unknown : { success: true };
    return { content: [{ type: 'text', text: this.truncate(JSON.stringify(data, null, 2)) }], isError: false };
  }
