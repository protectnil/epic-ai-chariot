...
export class GoogleTranslateMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoogleTranslateConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Cloud Translation: configuration object is required');
    }
    for (const __k of (['accessToken', 'projectId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Cloud Translation: ' + __k + ' is required');
      }
    }
