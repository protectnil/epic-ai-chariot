...
export class TomTomMCPServer extends MCPAdapterBase {
  ...
  private async batchGeocode(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try { addresses = JSON.parse(args.addresses as string) as string[]; } catch { return { content: [{ type: 'text', text: 'addresses must be a valid JSON array string' }], isError: true }; }
  ...
  private async searchAlongRoute(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    let route: string[];
    try { route = JSON.parse(args.route_points as string) as string[]; } catch { return { content: [{ type: 'text', text: 'route_points must be a valid JSON array string' }], isError: true }; }
