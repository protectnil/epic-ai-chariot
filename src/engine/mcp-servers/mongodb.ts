...
export class MongoDBMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MongoDBConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('MongoDB: configuration object is required');
    }
    for (const __k of (['appId', 'apiKey', 'dataSource', 'database'] as Array<keyof MongoDBConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('MongoDB: ' + __k + ' is required');
      }
    }
