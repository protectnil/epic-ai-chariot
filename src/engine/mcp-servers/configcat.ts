...
export class ConfigCatMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ConfigCatConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ConfigCat: configuration object is required');
    }
    for (const __k of (['basicUsername', 'basicPassword'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ConfigCat: ' + __k + ' is required');
      }
    }
