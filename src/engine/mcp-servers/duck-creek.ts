...
export class DuckCreekMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DuckCreekConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('duck-creek: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('duck-creek: ' + __k + ' is required');
      }
    }
  ...
  private async updatePolicy(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try { fields = JSON.parse(args.fields as string) as Record<string, unknown>; } catch { return { content: [{ type: 'text', text: 'fields must be a valid JSON string' }], isError: true }; }
  ...
  private async endorsePolicy(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try { changes = JSON.parse(args.changes as string) as Record<string, unknown>; } catch { return { content: [{ type: 'text', text: 'changes must be a valid JSON string' }], isError: true }; }
    const body: Record<string, unknown> = {
      effectiveDate: args.effective_date,
      changes,
    };
  ...
  private async getRateQuote(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try { riskData = JSON.parse(args.risk_data as string) as Record<string, unknown>; } catch { return { content: [{ type: 'text', text: 'risk_data must be a valid JSON string' }], isError: true }; }
