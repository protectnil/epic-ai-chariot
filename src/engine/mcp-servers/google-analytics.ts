...
export class GoogleAnalyticsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoogleAnalyticsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Analytics: configuration object is required');
    }
    for (const __k of (['accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Analytics: ' + __k + ' is required');
      }
    }
  ...
  private async checkCompatibility(args: Record<string, unknown>): Promise<ToolResult> {
    if (!args.property_id) {
      return { content: [{ type: 'text', text: 'property_id is required' }], isError: true };
    }
    const body: Record<string, unknown> = {};
