...
export class WorkdayMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WorkdayConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Workday: configuration object is required');
    }
    for (const __k of (['baseUrl', 'tenant', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Workday: ' + __k + ' is required');
      }
    }
