...
export class RedisMCPServer extends MCPAdapterBase {
  ...
  constructor(config: RedisConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Redis (Upstash): configuration object is required');
    }
    for (const __k of (['endpoint', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Redis (Upstash): ' + __k + ' is required');
      }
    }
