...
export class TheNounProjectMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TheNounProjectConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('The Noun Project: configuration object is required');
    }
    for (const __k of (['authHeader'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('The Noun Project: ' + __k + ' is required');
      }
    }
