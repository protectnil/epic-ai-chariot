...
export class ClearBladeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ClearBladeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ClearBlade: configuration object is required');
    }
    for (const __k of (['userToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ClearBlade: ' + __k + ' is required');
      }
    }
