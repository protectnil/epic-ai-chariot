...
export class MimecastMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MimecastConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mimecast: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret'] as Array<keyof MimecastConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Mimecast: ' + __k + ' is required');
      }
    }
