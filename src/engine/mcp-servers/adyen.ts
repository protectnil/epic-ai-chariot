...
export class AdyenMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AdyenConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Adyen: configuration object is required');
    }
    for (const __k of (['apiKey', 'merchantAccount'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Adyen: ' + __k + ' is required');
      }
    }
