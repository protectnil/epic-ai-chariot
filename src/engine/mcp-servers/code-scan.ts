...
export class CodeScanMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CodeScanConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('CodeScan: configuration object is required');
    }
    for (const __k of (['subscriptionId', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CodeScan: ' + __k + ' is required');
      }
    }
