...
export class ActiveCampaignMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ActiveCampaignConfig) {
    super();
    if (config === null) { throw new Error('ActiveCampaignMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('ActiveCampaign: configuration object is required');
    }
    for (const __k of (['apiKey', 'accountUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ActiveCampaign: ' + __k + ' is required');
      }
    }
