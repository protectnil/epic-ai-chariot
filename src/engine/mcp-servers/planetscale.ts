...
export class PlanetScaleMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PlanetScaleConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('PlanetScale: configuration object is required');
    }
    for (const __k of (['serviceTokenId', 'serviceToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('PlanetScale: ' + __k + ' is required');
      }
    }
  ...
  private async del(path: string): Promise<ToolResult> {
    ...
    const data: unknown = text ? JSON.parse(text) as unknown : { deleted: true };
    return { content: [{ type: 'text', text: this.truncate(data) }], isError: false };
  }
