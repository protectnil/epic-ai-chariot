...
export class TruAnonMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TruAnonConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('TruAnon: configuration object is required');
    }
    for (const __k of (['privateToken', 'service'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('TruAnon: ' + __k + ' is required');
      }
    }
