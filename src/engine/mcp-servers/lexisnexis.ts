...
export class LexisNexisMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LexisNexisConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('LexisNexis: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'baseUrl'] as Array<keyof LexisNexisConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('LexisNexis: ' + __k + ' is required');
      }
    }
