...
export class WowzaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WowzaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Wowza: configuration object is required');
    }
    for (const __k of (['apiKey', 'accessKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Wowza: ' + __k + ' is required');
      }
    }
