...
export class SanityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SanityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Sanity: configuration object is required');
    }
    for (const __k of (['projectId', 'dataset', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sanity: ' + __k + ' is required');
      }
    }
