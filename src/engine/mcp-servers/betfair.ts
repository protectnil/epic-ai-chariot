...
export class BetfairMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BetfairConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Betfair: configuration object is required');
    }
    for (const __k of (['sessionToken', 'appKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Betfair: ' + __k + ' is required');
      }
    }
  ...
  private streamSubscribeMarkets(args: Record<string, unknown>): ToolResult {
