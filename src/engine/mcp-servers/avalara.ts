...
export class AvalaraMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AvalaraConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Avalara AvaTax: configuration object is required');
    }
    for (const __k of (['accountId', 'licenseKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Avalara AvaTax: ' + __k + ' is required');
      }
    }
