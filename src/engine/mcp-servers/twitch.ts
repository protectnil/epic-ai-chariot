...
export class TwitchMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TwitchConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Twitch: configuration object is required');
    }
    for (const __k of (['clientId', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Twitch: ' + __k + ' is required');
      }
    }
