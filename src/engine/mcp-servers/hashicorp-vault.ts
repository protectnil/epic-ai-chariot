...
export class HashiCorpVaultMCPServer extends MCPAdapterBase {
  ...
  constructor(config: HashiCorpVaultConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('HashiCorp Vault: configuration object is required');
    }
    for (const __k of (['vaultAddr', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('HashiCorp Vault: ' + __k + ' is required');
      }
    }
