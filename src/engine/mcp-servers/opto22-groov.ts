...
export class Opto22GroovMCPServer extends MCPAdapterBase {
  ...
  constructor(config: Opto22GroovConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Opto 22 groov View: configuration object is required');
    }
    for (const __k of (['groovHost', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Opto 22 groov View: ' + __k + ' is required');
      }
    }
