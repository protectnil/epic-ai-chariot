...
export class CrowdStrikeIdentityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CrowdStrikeIdentityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('CrowdStrike Falcon Identity Protection: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CrowdStrike Falcon Identity Protection: ' + __k + ' is required');
      }
    }
