...
export class TinesMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TinesConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Tines: configuration object is required');
    }
    for (const __k of (['apiKey', 'tenantUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Tines: ' + __k + ' is required');
      }
    }
