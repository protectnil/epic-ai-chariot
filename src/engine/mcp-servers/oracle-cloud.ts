...
export class OracleCloudMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OracleCloudConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Oracle Cloud Infrastructure: configuration object is required');
    }
    for (const __k of (['authToken', 'region', 'tenancyId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Oracle Cloud Infrastructure: ' + __k + ' is required');
      }
    }
