...
export class SumoLogicMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SumoLogicConfig) {
    super();
    if (config === null) { throw new Error('SumoLogicMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Sumo Logic: configuration object is required');
    }
    for (const __k of (['accessId', 'accessKey', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sumo Logic: ' + __k + ' is required');
      }
    }
    if (config.baseUrl) {
