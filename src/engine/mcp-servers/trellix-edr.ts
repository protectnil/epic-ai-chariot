...
export class TrellixEDRMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TrellixEDRConfig) {
    super();
    if (config === null) { throw new Error('TrellixEDRMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Trellix EDR: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'apiKey', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Trellix EDR: ' + __k + ' is required');
      }
    }
