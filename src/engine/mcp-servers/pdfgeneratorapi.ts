...
export class PDFGeneratorAPIMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PDFGeneratorAPIConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('PDF Generator API: configuration object is required');
    }
    for (const __k of (['jwtToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('PDF Generator API: ' + __k + ' is required');
      }
    }
