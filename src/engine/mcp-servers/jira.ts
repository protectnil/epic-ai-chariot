...
export class JiraMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JiraConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Jira: configuration object is required');
    }
    for (const __k of (['instance', 'email', 'api_token'] as Array<keyof JiraConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Jira: ' + __k + ' is required');
      }
    }
