...
export class WalmartOrderMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WalmartOrderConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Walmart Marketplace Orders: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Walmart Marketplace Orders: ' + __k + ' is required');
      }
    }
