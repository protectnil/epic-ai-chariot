...
export class SauceLabsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SauceLabsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Sauce Labs: configuration object is required');
    }
    for (const __k of (['username', 'accessKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sauce Labs: ' + __k + ' is required');
      }
    }
