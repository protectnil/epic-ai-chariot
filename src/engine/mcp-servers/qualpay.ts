...
export class QualpayMCPServer extends MCPAdapterBase {
  ...
  constructor(config: QualpayConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Qualpay: configuration object is required');
    }
    for (const __k of (['merchantId', 'securityKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Qualpay: ' + __k + ' is required');
      }
    }
