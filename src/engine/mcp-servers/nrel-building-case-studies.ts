...
export class NrelBuildingCaseStudiesMCPServer extends MCPAdapterBase {
  ...
  constructor(config: NrelBuildingCaseStudiesConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('NREL High Performance Building Database: configuration object is required');
    }
    for (const __k of (['apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('NREL High Performance Building Database: ' + __k + ' is required');
      }
    }
