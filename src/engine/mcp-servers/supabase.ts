...
export class SupabaseMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SupabaseConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Supabase: configuration object is required');
    }
    for (const __k of (['project', 'anonKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Supabase: ' + __k + ' is required');
      }
    }
  ...
  protected override truncate(data: unknown): string {
  ...
  private async listTables(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.dataBaseUrl}/`, { headers });
    if (!response.ok) {
  ...
  private async executeSql(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects/${ref}/database/query`, {
      method: 'POST',
  ...
  private async listStorageBuckets(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const key = useServiceRole && this.serviceRoleKey ? this.serviceRoleKey : this.anonKey;
    const response = await this.fetchWithRetry(`${baseStorageUrl}/bucket`, {
  ...
  private async listStorageObjects(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const baseStorageUrl = `https://${this.project}.supabase.co/storage/v1`;
    const key = useServiceRole && this.serviceRoleKey ? this.serviceRoleKey : this.anonKey;
    const response = await this.fetchWithRetry(`${baseStorageUrl}/object/list/${encodeURIComponent(args.bucket_id as string)}`, {
      method: 'POST',
  ...
  private async listAuthUsers(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects/${ref}/auth/users?${params}`, {
      headers: this.mgmtHeaders(),
    });
    let data: unknown;
    try { data = await response.json(); } catch { data = { status: response.status, statusText: response.statusText }; }
  ...
  private async getAuthUser(args: Record<string, unknown>): Promise<ToolResult> {
    const ref = this.project;
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects/${ref}/auth/users/${encodeURIComponent(args.user_id as string)}`, {
      headers: this.mgmtHeaders(),
    });
    let data: unknown;
    try { data = await response.json(); } catch { data = { status: response.status, statusText: response.statusText }; }
  ...
  private async listProjects(): Promise<ToolResult> {
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects`, { headers: this.mgmtHeaders() });
    let data: unknown;
    try { data = await response.json(); } catch { data = { status: response.status, statusText: response.statusText }; }
    return { content: [{ type: 'text', text: this.truncate(data) }], isError: !response.ok };
  }
  ...
  private async getProject(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects/${encodeURIComponent(ref)}`, { headers: this.mgmtHeaders() });
    let data: unknown;
    try { data = await response.json(); } catch { data = { status: response.status, statusText: response.statusText }; }
    return { content: [{ type: 'text', text: this.truncate(data) }], isError: !response.ok };
  }
  ...
  private async getProjectHealth(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.mgmtBaseUrl}/projects/${encodeURIComponent(ref)}/health${query}`, {
      headers: this.mgmtHeaders(),
    });
    let data: unknown;
    try { data = await response.json(); } catch { data = { status: response.status, statusText: response.statusText }; }
