...
export class OdooMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OdooConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Odoo ERP: configuration object is required');
    }
    for (const __k of (['baseUrl', 'database', 'username', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Odoo ERP: ' + __k + ' is required');
      }
    }
