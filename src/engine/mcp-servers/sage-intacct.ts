...
export class SageIntacctMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SageIntacctConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Sage Intacct: configuration object is required');
    }
    for (const __k of (['senderId', 'senderPassword', 'companyId', 'userId', 'userPassword'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sage Intacct: ' + __k + ' is required');
      }
    }
