...
export class GoogleHomeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoogleHomeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Home: configuration object is required');
    }
    for (const __k of (['localAuthToken', 'deviceIp'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Home: ' + __k + ' is required');
      }
    }
