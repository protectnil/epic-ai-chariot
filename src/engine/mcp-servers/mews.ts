...
export class MewsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MewsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mews: configuration object is required');
    }
    for (const __k of (['clientToken', 'accessToken'] as Array<keyof MewsConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Mews: ' + __k + ' is required');
      }
    }
