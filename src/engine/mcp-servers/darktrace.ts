...
export class DarktraceMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DarktraceConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('darktrace: configuration object is required');
    }
    for (const __k of (['publicToken', 'privateToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('darktrace: ' + __k + ' is required');
      }
    }
