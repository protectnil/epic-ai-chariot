...
export class CanadaPostMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CanadaPostConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Canada Post: configuration object is required');
    }
    for (const __k of (['apiUsername', 'apiPassword'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Canada Post: ' + __k + ' is required');
      }
    }
