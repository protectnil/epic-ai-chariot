...
export class JenkinsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JenkinsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Jenkins: configuration object is required');
    }
    for (const __k of (['baseUrl', 'username', 'apiToken'] as Array<keyof JenkinsConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Jenkins: ' + __k + ' is required');
      }
    }
