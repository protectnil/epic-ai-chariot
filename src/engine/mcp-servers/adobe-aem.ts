...
export class AdobeAemMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AdobeAemConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Adobe Experience Manager (AEM): configuration object is required');
    }
    for (const __k of (['baseUrl', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Adobe Experience Manager (AEM): ' + __k + ' is required');
      }
    }
