...
export class KlarnaPaymentsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: KlarnaPaymentsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Klarna Payments: configuration object is required');
    }
    for (const __k of (['apiUser', 'apiPassword'] as Array<keyof KlarnaPaymentsConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Klarna Payments: ' + __k + ' is required');
      }
    }
