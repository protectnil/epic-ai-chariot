...
export class PrometheusMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PrometheusConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Prometheus: configuration object is required');
    }
    if (!config.baseUrl && (!config.host || config.host === '')) {
      throw new Error('Prometheus: host is required when baseUrl is not provided');
    }
