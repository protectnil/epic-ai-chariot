...
export class BlackbaudMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BlackbaudConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Blackbaud: configuration object is required');
    }
    for (const __k of (['accessToken', 'subscriptionKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Blackbaud: ' + __k + ' is required');
      }
    }
  ...
  private listGiftTypes(): ToolResult {
