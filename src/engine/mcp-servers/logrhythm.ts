...
export class LogRhythmMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LogRhythmConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('LogRhythm: configuration object is required');
    }
    for (const __k of (['host', 'apiKey'] as Array<keyof LogRhythmConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('LogRhythm: ' + __k + ' is required');
      }
    }
