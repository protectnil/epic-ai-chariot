...
export class OktaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OktaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Okta: configuration object is required');
    }
    for (const __k of (['orgUrl', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Okta: ' + __k + ' is required');
      }
    }
