...
export class CivicPlusMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CivicPlusConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('CivicPlus: configuration object is required');
    }
    for (const __k of (['apiToken', 'organizationId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CivicPlus: ' + __k + ' is required');
      }
    }
