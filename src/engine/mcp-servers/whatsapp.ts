...
export class WhatsAppMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WhatsAppConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('WhatsApp Business API: configuration object is required');
    }
    for (const __k of (['baseUrl', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('WhatsApp Business API: ' + __k + ' is required');
      }
    }
