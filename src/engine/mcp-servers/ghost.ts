...
export class GhostMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GhostConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Ghost: configuration object is required');
    }
    for (const __k of (['adminApiKey', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Ghost: ' + __k + ' is required');
      }
    }
