...
export class GoogleAdsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoogleAdsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Ads: configuration object is required');
    }
    for (const __k of (['accessToken', 'developerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Ads: ' + __k + ' is required');
      }
    }
