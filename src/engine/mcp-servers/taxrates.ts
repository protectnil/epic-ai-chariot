...
export class TaxRatesMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TaxRatesConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Taxrates.io: configuration object is required');
    }
    for (const __k of (['domain'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Taxrates.io: ' + __k + ' is required');
      }
    }
