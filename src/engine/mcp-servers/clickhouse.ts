...
export class ClickHouseMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ClickHouseConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ClickHouse: configuration object is required');
    }
    for (const __k of (['user', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ClickHouse: ' + __k + ' is required');
      }
    }
