...
export class RedoxMCPServer extends MCPAdapterBase {
  ...
  constructor(config: RedoxConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Redox: configuration object is required');
    }
    for (const __k of (['clientId', 'privateKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Redox: ' + __k + ' is required');
      }
    }
