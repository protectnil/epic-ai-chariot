...
export class PayRunMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PayRunConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('PayRun.IO: configuration object is required');
    }
    for (const __k of (['authorizationHeader'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('PayRun.IO: ' + __k + ' is required');
      }
    }
