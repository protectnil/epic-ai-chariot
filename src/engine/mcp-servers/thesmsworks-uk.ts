...
export class TheSMSWorksUKMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TheSMSWorksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('The SMS Works: configuration object is required');
    }
    for (const __k of (['jwt'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('The SMS Works: ' + __k + ' is required');
      }
    }
    this.jwt = config.jwt;
