...
export class CyberArkMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CyberArkConfig) {
    super();
    if (config === null) { throw new Error('CyberArkMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('CyberArk PAM: configuration object is required');
    }
    for (const __k of (['username', 'password', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CyberArk PAM: ' + __k + ' is required');
      }
    }
