...
export class DatabricksMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DatabricksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('databricks: configuration object is required');
    }
    for (const __k of (['host', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('databricks: ' + __k + ' is required');
      }
    }
