...
export class BrightcoveMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BrightcoveConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Brightcove: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'accountId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Brightcove: ' + __k + ' is required');
      }
    }
