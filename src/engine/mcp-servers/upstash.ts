...
export class UpstashMCPServer extends MCPAdapterBase {
  ...
  constructor(config: UpstashConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Upstash: configuration object is required');
    }
    for (const __k of (['email', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Upstash: ' + __k + ' is required');
      }
    }
  ...
  private async apiDelete(path: string): Promise<ToolResult> {
    ...
    const data: unknown = text ? JSON.parse(text) as unknown : { status: 'deleted' };
    return { content: [{ type: 'text', text: this.truncate(data) }], isError: false };
  }
