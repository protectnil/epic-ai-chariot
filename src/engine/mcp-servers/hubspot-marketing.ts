...
export class HubSpotMarketingMCPServer extends MCPAdapterBase {
  ...
  constructor(config: HubSpotMarketingConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('HubSpot Marketing Hub: configuration object is required');
    }
    for (const __k of (['privateAppToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('HubSpot Marketing Hub: ' + __k + ' is required');
      }
    }
