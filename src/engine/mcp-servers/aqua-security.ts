...
export class AquaSecurityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AquaSecurityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Aqua Security: configuration object is required');
    }
    for (const __k of (['aquaKey', 'aquaSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Aqua Security: ' + __k + ' is required');
      }
    }
