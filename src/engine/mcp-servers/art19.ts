...
export class Art19MCPServer extends MCPAdapterBase {
  ...
  constructor(config: Art19Config) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ART19: configuration object is required');
    }
    for (const __k of (['token', 'credential'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ART19: ' + __k + ' is required');
      }
    }
