...
export class ProofpointMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ProofpointConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Proofpoint TAP: configuration object is required');
    }
    for (const __k of (['servicePrincipal', 'secret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Proofpoint TAP: ' + __k + ' is required');
      }
    }
