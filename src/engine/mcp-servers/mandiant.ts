...
export class MandiantMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MandiantConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Mandiant: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'appName'] as Array<keyof MandiantConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Mandiant: ' + __k + ' is required');
      }
    }
