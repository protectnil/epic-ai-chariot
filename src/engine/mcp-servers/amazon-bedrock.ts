...
async function awsSigV4Sign(
  ...
  const canonicalHeaders = sortedKeys
    .map(lk => {
      ...
      if (!origKey) throw new Error(`invariant: canonical header key ${lk} not found`);
...
export class AmazonBedrockMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BedrockConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Amazon Bedrock: configuration object is required');
    }
    for (const __k of (['region', 'accessKeyId', 'secretAccessKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Amazon Bedrock: ' + __k + ' is required');
      }
    }
