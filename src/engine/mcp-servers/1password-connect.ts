...
export class OnePasswordConnectMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OnePasswordConnectConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('1Password Connect: configuration object is required');
    }
    for (const __k of (['connectToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('1Password Connect: ' + __k + ' is required');
      }
    }
