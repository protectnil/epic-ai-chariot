...
export class MorningstarMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MorningstarConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Morningstar: configuration object is required');
    }
    for (const __k of (['username', 'password'] as Array<keyof MorningstarConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Morningstar: ' + __k + ' is required');
      }
    }
