...
export class StytchMCPServer extends MCPAdapterBase {
  ...
  constructor(config: StytchConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Stytch: configuration object is required');
    }
    for (const __k of (['projectId', 'secret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Stytch: ' + __k + ' is required');
      }
    }
