...
export class ZendeskMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ZendeskConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Zendesk: configuration object is required');
    }
    for (const __k of (['email', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Zendesk: ' + __k + ' is required');
      }
    }
