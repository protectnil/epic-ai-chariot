...
export class CornerstoneMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CornerstoneConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Cornerstone OnDemand: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'corpname'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Cornerstone OnDemand: ' + __k + ' is required');
      }
    }
