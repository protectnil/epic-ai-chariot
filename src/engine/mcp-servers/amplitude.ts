...
export class AmplitudeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AmplitudeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Amplitude: configuration object is required');
    }
    for (const __k of (['apiKey', 'secretKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Amplitude: ' + __k + ' is required');
      }
    }
