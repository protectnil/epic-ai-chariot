...
export class AfterbanksMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AfterbanksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Afterbanks: configuration object is required');
    }
    for (const __k of (['servicekey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Afterbanks: ' + __k + ' is required');
      }
    }
