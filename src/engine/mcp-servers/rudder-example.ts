...
export class RudderExampleMCPServer extends MCPAdapterBase {
  ...
  constructor(config: RudderConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Rudder: configuration object is required');
    }
    for (const __k of (['apiToken', 'rudderServer'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Rudder: ' + __k + ' is required');
      }
    }
