...
export class LjaeroDflightV100MCPServer extends MCPAdapterBase {
  ...
  constructor(config: LjaeroDflightConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('LJAero DFlight: configuration object is required');
    }
    if (config.apiKey === undefined || config.apiKey === null || config.apiKey === '') {
      throw new Error('LJAero DFlight: apiKey is required');
    }
  ...
  async callTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      switch (name) {
        ...
        case 'query_weather_by_distance': {
          const body: Record<string, unknown> = { latitude: args.latitude, longitude: args.longitude, distance: args.distance };
          if (args.wxtypes) body['wxtypes'] = args.wxtypes;
          if (args.hours !== undefined) body['hours'] = args.hours;
          return this.post('/us/v1/wx-forecast/distance-query', body);
        }
        case 'query_weather_by_polygon': {
          const body: Record<string, unknown> = { poly: args.poly };
          if (args.wxtypes) body['wxtypes'] = args.wxtypes;
          if (args.hours !== undefined) body['hours'] = args.hours;
          return this.post('/us/v1/wx-forecast/polygon-query', body);
        }
        case 'query_weather_by_route': {
          const body: Record<string, unknown> = { route: args.route };
          if (args.wxtypes) body['wxtypes'] = args.wxtypes;
          if (args.hours !== undefined) body['hours'] = args.hours;
          return this.post('/us/v1/wx-forecast/route-query', body);
        }
        default:
