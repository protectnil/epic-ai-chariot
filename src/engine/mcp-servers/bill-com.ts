...
export class BillComMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BillComConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('BILL (Bill.com): configuration object is required');
    }
    for (const __k of (['userName', 'password', 'organizationId', 'devKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('BILL (Bill.com): ' + __k + ' is required');
      }
    }
