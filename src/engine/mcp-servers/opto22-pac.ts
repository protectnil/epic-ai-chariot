...
export class Opto22PACMCPServer extends MCPAdapterBase {
  ...
  constructor(config: Opto22PACConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Opto 22 PAC Control: configuration object is required');
    }
    for (const __k of (['controllerHost', 'apiKeyId', 'apiKeyValue'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Opto 22 PAC Control: ' + __k + ' is required');
      }
    }
