...
export class PlivoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PlivoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Plivo: configuration object is required');
    }
    for (const __k of (['authId', 'authToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Plivo: ' + __k + ' is required');
      }
    }
