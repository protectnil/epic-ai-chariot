...
export class CiscoDuoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CiscoDuoConfig) {
    super();
    if (config === null) { throw new Error('CiscoDuoMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Cisco Duo: configuration object is required');
    }
    for (const __k of (['integrationKey', 'secretKey', 'apiHost'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Cisco Duo: ' + __k + ' is required');
      }
    }
