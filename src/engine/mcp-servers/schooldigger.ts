...
export class SchoolDiggerMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SchoolDiggerConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('SchoolDigger: configuration object is required');
    }
    for (const __k of (['appID', 'appKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('SchoolDigger: ' + __k + ' is required');
      }
    }
