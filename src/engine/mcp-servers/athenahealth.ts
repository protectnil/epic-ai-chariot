...
export class AthenaHealthMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AthenaHealthConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Athenahealth: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'practiceId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Athenahealth: ' + __k + ' is required');
      }
    }
