...
export class SiemensMindSphereMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SiemensMindSphereConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Siemens MindSphere: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'tenant'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Siemens MindSphere: ' + __k + ' is required');
      }
    }
