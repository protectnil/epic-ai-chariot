...
export class ArgoCDMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ArgoCDConfig) {
    super();
    if (config === null) { throw new Error('ArgoCDMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Argo CD: configuration object is required');
    }
    for (const __k of (['token', 'server'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Argo CD: ' + __k + ' is required');
      }
    }
