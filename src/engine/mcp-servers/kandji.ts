...
export class KandjiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: KandjiConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Kandji: configuration object is required');
    }
    for (const __k of (['apiToken', 'baseUrl'] as Array<keyof KandjiConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Kandji: ' + __k + ' is required');
      }
    }
