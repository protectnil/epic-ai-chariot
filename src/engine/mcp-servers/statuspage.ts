...
export class StatuspageMCPServer extends MCPAdapterBase {
  ...
  constructor(config: StatuspageConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Statuspage: configuration object is required');
    }
    for (const __k of (['apiKey', 'pageId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Statuspage: ' + __k + ' is required');
      }
    }
