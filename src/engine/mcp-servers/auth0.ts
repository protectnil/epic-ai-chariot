...
export class Auth0MCPServer extends MCPAdapterBase {
  ...
  constructor(config: Auth0Config) {
    super();
    if (config === null) { throw new Error('Auth0MCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Auth0: configuration object is required');
    }
    for (const __k of (['managementToken', 'domain'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Auth0: ' + __k + ' is required');
      }
    }
