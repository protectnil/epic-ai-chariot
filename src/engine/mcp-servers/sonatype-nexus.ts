...
export class SonatypeNexusMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SonatypeNexusConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Sonatype Nexus: configuration object is required');
    }
    for (const __k of (['baseUrl', 'username', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Sonatype Nexus: ' + __k + ' is required');
      }
    }
