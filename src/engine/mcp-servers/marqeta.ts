...
export class MarqetaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MarqetaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Marqeta: configuration object is required');
    }
    for (const __k of (['applicationToken', 'accessToken'] as Array<keyof MarqetaConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Marqeta: ' + __k + ' is required');
      }
    }
