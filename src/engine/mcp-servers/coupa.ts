...
export class CoupaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CoupaConfig) {
    super();
    if (config === null) { throw new Error('CoupaMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Coupa: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'instanceUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Coupa: ' + __k + ' is required');
      }
    }
