...
export class PandaScoreMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PandaScoreConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('PandaScore: configuration object is required');
    }
    for (const __k of (['token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('PandaScore: ' + __k + ' is required');
      }
    }
  ...
  private static readonly PAGE_KEY_MAP: Record<string, string> = {
    page: 'page[number]',
    per_page: 'page[size]',
  };
  ...
  private buildParams(args: Record<string, unknown>, allowed: string[]): URLSearchParams {
    ...
    for (const key of allowed) {
      if (args[key] !== undefined && args[key] !== null) {
        params.set(PandaScoreMCPServer.PAGE_KEY_MAP[key] ?? key, String(args[key]));
      }
    }
    return params;
  }
