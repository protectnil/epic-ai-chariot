...
export class Text2DataMCPServer extends MCPAdapterBase {
  ...
  constructor(config: Text2DataConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Text2Data: configuration object is required');
    }
    for (const __k of (['privateKey', 'secret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Text2Data: ' + __k + ' is required');
      }
    }
