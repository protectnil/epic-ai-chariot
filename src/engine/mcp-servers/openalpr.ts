...
export class OpenALPRMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenALPRConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('OpenALPR: configuration object is required');
    }
    for (const __k of (['secretKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('OpenALPR: ' + __k + ' is required');
      }
    }
