...
export class VeevaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VeevaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Veeva Vault: configuration object is required');
    }
    for (const __k of (['username', 'password', 'vaultDomain'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Veeva Vault: ' + __k + ' is required');
      }
    }
