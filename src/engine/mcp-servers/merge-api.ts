...
export class MergeApiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MergeApiConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Merge API: configuration object is required');
    }
    if (config.apiKey === undefined || config.apiKey === null || config.apiKey === '') {
      throw new Error('Merge API: apiKey is required');
    }
  ...
  private async forceResync(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (!this.accountToken) {
      return { content: [{ type: 'text', text: 'X-Account-Token is required for force resync operations' }], isError: true };
    }
