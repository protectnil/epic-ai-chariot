...
export class AzureBlobMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AzureBlobConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Azure Blob Storage: configuration object is required');
    }
    for (const __k of (['tenantId', 'clientId', 'clientSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Azure Blob Storage: ' + __k + ' is required');
      }
    }
