...
export class TableauMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TableauConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Tableau: configuration object is required');
    }
    for (const __k of (['serverUrl', 'apiVersion', 'patName', 'patSecret', 'siteContentUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Tableau: ' + __k + ' is required');
      }
    }
