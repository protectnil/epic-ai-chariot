...
export class XeroMCPServer extends MCPAdapterBase {
  ...
  constructor(config: XeroConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Xero: configuration object is required');
    }
    for (const __k of (['accessToken', 'tenantId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Xero: ' + __k + ' is required');
      }
    }
