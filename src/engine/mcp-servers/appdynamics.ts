...
export class AppDynamicsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AppDynamicsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Cisco AppDynamics: configuration object is required');
    }
    for (const __k of (['controllerUrl', 'username', 'accountName', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Cisco AppDynamics: ' + __k + ' is required');
      }
    }
