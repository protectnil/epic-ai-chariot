...
export class ElasticSecurityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ElasticSecurityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('elastic-security: configuration object is required');
    }
    for (const __k of (['kibanaUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('elastic-security: ' + __k + ' is required');
      }
    }
