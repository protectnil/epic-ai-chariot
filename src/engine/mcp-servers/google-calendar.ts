...
// The official Google Workspace CLI (googleworkspace/cli, Elastic-2.0, active 2026) exposes
...
export class GoogleCalendarMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoogleCalendarConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google Calendar: configuration object is required');
    }
    for (const __k of (['accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Google Calendar: ' + __k + ' is required');
      }
    }
