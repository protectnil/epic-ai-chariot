...
export class OpenWeatherMapMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenWeatherMapConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('OpenWeatherMap: configuration object is required');
    }
    for (const __k of (['apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('OpenWeatherMap: ' + __k + ' is required');
      }
    }
  ...
  private getWeatherMapTileUrl(args: Record<string, unknown>): ToolResult {
