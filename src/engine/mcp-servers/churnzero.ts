...
export class ChurnZeroMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ChurnZeroConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Churn Zero: configuration object is required');
    }
    for (const __k of (['instance', 'email', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Churn Zero: ' + __k + ' is required');
      }
    }
