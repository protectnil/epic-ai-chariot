...
export class MicrosoftEntraMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MicrosoftEntraConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Microsoft Entra ID: configuration object is required');
    }
    for (const __k of (['tenantId', 'clientId', 'clientSecret'] as Array<keyof MicrosoftEntraConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Microsoft Entra ID: ' + __k + ' is required');
      }
    }
