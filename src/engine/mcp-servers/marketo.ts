...
export class MarketoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MarketoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Marketo: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'identityUrl', 'restBaseUrl'] as Array<keyof MarketoConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Marketo: ' + __k + ' is required');
      }
    }
