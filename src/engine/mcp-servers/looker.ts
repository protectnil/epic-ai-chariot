...
export class LookerMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LookerConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Looker: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'baseUrl'] as Array<keyof LookerConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Looker: ' + __k + ' is required');
      }
    }
