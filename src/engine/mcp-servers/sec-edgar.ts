...
export class SecEdgarMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SecEdgarConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('SEC EDGAR: configuration object is required');
    }
    for (const __k of (['userAgent'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('SEC EDGAR: ' + __k + ' is required');
      }
    }
