...
export class FreshBooksMCPServer extends MCPAdapterBase {
  ...
  constructor(config: FreshBooksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('freshbooks: configuration object is required');
    }
    for (const __k of (['accessToken', 'accountId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('freshbooks: ' + __k + ' is required');
      }
    }
