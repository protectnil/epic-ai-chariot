...
export class SemantriaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SemantriaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Semantria: configuration object is required');
    }
    for (const __k of (['consumerKey', 'consumerSecret', 'configId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Semantria: ' + __k + ' is required');
      }
    }
