...
export class AWSTextractMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AWSTextractConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('AWS Textract: configuration object is required');
    }
    for (const __k of (['accessKeyId', 'secretAccessKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('AWS Textract: ' + __k + ' is required');
      }
    }
  ...
  private async analyzeDocument(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (args.queries) {
      try {
        payload['QueriesConfig'] = { Queries: JSON.parse(args.queries as string) as unknown[] };
      } catch {
