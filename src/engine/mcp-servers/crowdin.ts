...
export class CrowdinMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CrowdinConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Crowdin: configuration object is required');
    }
    for (const __k of (['personalToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Crowdin: ' + __k + ' is required');
      }
    }
