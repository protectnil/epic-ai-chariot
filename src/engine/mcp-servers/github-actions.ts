...
export class GitHubActionsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GitHubActionsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('GitHub Actions: configuration object is required');
    }
    for (const __k of (['apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('GitHub Actions: ' + __k + ' is required');
      }
    }
  ...
  private async ghPut(path: string, body: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (response.status === 204 || response.status === 201) {
      ...
      const result: unknown = text ? JSON.parse(text) as unknown : { success: true, status: response.status };
