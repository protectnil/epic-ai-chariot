...
export class BeyondTrustMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BeyondTrustConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('BeyondTrust Password Safe: configuration object is required');
    }
    for (const __k of (['apiKey', 'username', 'password', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('BeyondTrust Password Safe: ' + __k + ' is required');
      }
    }
