...
export class WindowsBatchBatchserviceMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WindowsBatchConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Windows Azure Batch Service: configuration object is required');
    }
    for (const __k of (['accessToken', 'accountEndpoint'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Windows Azure Batch Service: ' + __k + ' is required');
      }
    }
