...
export class ExpensifyMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ExpensifyConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('expensify: configuration object is required');
    }
    for (const __k of (['partnerUserID', 'partnerUserSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('expensify: ' + __k + ' is required');
      }
    }
