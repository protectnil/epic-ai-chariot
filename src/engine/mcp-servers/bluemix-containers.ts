...
export class BluemixContainersMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BluemixContainersConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('IBM Containers (Bluemix): configuration object is required');
    }
    for (const __k of (['authToken', 'projectId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('IBM Containers (Bluemix): ' + __k + ' is required');
      }
    }
