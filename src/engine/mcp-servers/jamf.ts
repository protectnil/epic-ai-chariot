...
export class JamfMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JamfConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Jamf Pro: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'baseUrl'] as Array<keyof JamfConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Jamf Pro: ' + __k + ' is required');
      }
    }
