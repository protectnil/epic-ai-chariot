...
export class InfogenesisMCPServer extends MCPAdapterBase {
  ...
  constructor(config: InfogenesisConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Agilysys InfoGenesis POS: configuration object is required');
    }
    for (const __k of (['baseUrl', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Agilysys InfoGenesis POS: ' + __k + ' is required');
      }
    }
