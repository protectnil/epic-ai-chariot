...
export class MedalliaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MedalliaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Medallia: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'instance', 'company'] as Array<keyof MedalliaConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Medallia: ' + __k + ' is required');
      }
    }
