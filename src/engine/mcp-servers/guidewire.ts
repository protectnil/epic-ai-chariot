...
export class GuidewireMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GuidewireConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Guidewire PolicyCenter: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'tenantUrl', 'tokenUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Guidewire PolicyCenter: ' + __k + ' is required');
      }
    }
