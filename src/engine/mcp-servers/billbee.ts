...
export class BillbeeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BillbeeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Billbee: configuration object is required');
    }
    for (const __k of (['apiKey', 'username', 'apiPassword'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Billbee: ' + __k + ' is required');
      }
    }
