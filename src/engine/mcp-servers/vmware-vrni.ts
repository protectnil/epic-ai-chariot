...
export class VmwareVrniMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VrniConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('VMware vRealize Network Insight: configuration object is required');
    }
    for (const __k of (['baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('VMware vRealize Network Insight: ' + __k + ' is required');
      }
    }
