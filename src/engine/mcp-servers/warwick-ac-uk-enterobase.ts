...
export class WarwickAcUkEnterobaseMCPServer extends MCPAdapterBase {
  ...
  constructor(config: EnterobaseConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Enterobase: configuration object is required');
    }
    for (const __k of (['apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Enterobase: ' + __k + ' is required');
      }
    }
