...
export class PostgreSQLMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PostgreSQLConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('PostgreSQL: configuration object is required');
    }
    for (const __k of (['proxyUrl', 'apiKey', 'host', 'database', 'user', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('PostgreSQL: ' + __k + ' is required');
      }
    }
    if (config.port === undefined || config.port === null) {
      throw new Error('PostgreSQL: port is required');
    }
