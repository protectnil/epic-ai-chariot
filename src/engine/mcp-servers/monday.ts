...
export class MondayMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MondayConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Monday.com: configuration object is required');
    }
    if (config.apiToken === undefined || config.apiToken === null || config.apiToken === '') {
      throw new Error('Monday.com: apiToken is required');
    }
