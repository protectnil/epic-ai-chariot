...
export class SpectroCoinMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SpectroCoinConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('SpectroCoin: configuration object is required');
    }
    for (const __k of (['merchantId', 'apiId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('SpectroCoin: ' + __k + ' is required');
      }
    }
