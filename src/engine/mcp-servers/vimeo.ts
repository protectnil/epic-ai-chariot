...
export class VimeoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VimeoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Vimeo: configuration object is required');
    }
    for (const __k of (['accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Vimeo: ' + __k + ' is required');
      }
    }
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            filter: {
              type: 'string',
              description: 'Content filter: Creative Commons license code (CC, CC-BY, CC-BY-NC, CC-BY-NC-ND, CC-BY-NC-SA, CC-BY-ND, CC-BY-SA, public-domain), or leave blank for all',
            },
            sort: {
              type: 'string',
