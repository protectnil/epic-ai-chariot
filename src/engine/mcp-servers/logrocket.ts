...
export class LogRocketMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LogRocketConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('LogRocket: configuration object is required');
    }
    for (const __k of (['apiKey', 'organizationSlug', 'appSlug'] as Array<keyof LogRocketConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('LogRocket: ' + __k + ' is required');
      }
    }
