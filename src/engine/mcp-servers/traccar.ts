...
export class TraccarMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TraccarConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Traccar: configuration object is required');
    }
    for (const __k of (['email', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Traccar: ' + __k + ' is required');
      }
    }
