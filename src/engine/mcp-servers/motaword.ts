...
export class MotaWordMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MotaWordConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('MotaWord: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret'] as Array<keyof MotaWordConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('MotaWord: ' + __k + ' is required');
      }
    }
