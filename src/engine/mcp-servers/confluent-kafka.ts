...
export class ConfluentKafkaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ConfluentConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Confluent Kafka: configuration object is required');
    }
    for (const __k of (['apiKey', 'apiSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Confluent Kafka: ' + __k + ' is required');
      }
    }
  ...
  private kafkaClusterIdChecked(): string {
    if (!this.kafkaClusterId) throw new Error('invariant: kafkaClusterId not set after requireKafkaRest');
  ...
  private async listTopics(): Promise<ToolResult> {
    ...
    return this.fetchJson(
      `${this.kafkaRestEndpoint}/kafka/v3/clusters/${encodeURIComponent(this.kafkaClusterIdChecked())}/topics`,
  ...
  private async deleteTopic(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(
      `${this.kafkaRestEndpoint}/kafka/v3/clusters/${encodeURIComponent(this.kafkaClusterIdChecked())}/topics/${encodeURIComponent(topicName)}`,
  ...
  private async getTopicConfig(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    return this.fetchJson(
      `${this.kafkaRestEndpoint}/kafka/v3/clusters/${encodeURIComponent(this.kafkaClusterIdChecked())}/topics/${encodeURIComponent(topicName)}/configs`,
      { method: 'GET', headers: this.kafkaHeaders() },
    );
  }
  ...
  private async listConsumerGroups(): Promise<ToolResult> {
    ...
    if (guard) return guard;
    return this.fetchJson(
      `${this.kafkaRestEndpoint}/kafka/v3/clusters/${encodeURIComponent(this.kafkaClusterIdChecked())}/consumer-groups`,
      { method: 'GET', headers: this.kafkaHeaders() },
    );
  }
  ...
  private async getConsumerGroup(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    return this.fetchJson(
      `${this.kafkaRestEndpoint}/kafka/v3/clusters/${encodeURIComponent(this.kafkaClusterIdChecked())}/consumer-groups/${encodeURIComponent(groupId)}`,
      { method: 'GET', headers: this.kafkaHeaders() },
    );
  }
