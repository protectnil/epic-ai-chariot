...
export class PingIdentityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PingIdentityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Ping Identity (PingOne): configuration object is required');
    }
    for (const __k of (['bearerToken', 'environmentId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Ping Identity (PingOne): ' + __k + ' is required');
      }
    }
