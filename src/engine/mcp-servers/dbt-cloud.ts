...
export class DbtCloudMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DbtCloudConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('dbt-cloud: configuration object is required');
    }
    for (const __k of (['serviceToken', 'accountId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('dbt-cloud: ' + __k + ' is required');
      }
    }
