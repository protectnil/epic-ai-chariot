...
export class ExperianMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ExperianConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('experian: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'username', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('experian: ' + __k + ' is required');
      }
    }
