...
export class JavatpointMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JavatpointFCMConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Firebase Cloud Messaging (FCM): configuration object is required');
    }
    for (const __k of (['projectId', 'accessToken'] as Array<keyof JavatpointFCMConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Firebase Cloud Messaging (FCM): ' + __k + ' is required');
      }
    }
