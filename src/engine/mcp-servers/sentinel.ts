...
export class SentinelMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SentinelConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Microsoft Sentinel: configuration object is required');
    }
    for (const __k of (['subscriptionId', 'resourceGroup', 'workspaceName', 'bearerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Microsoft Sentinel: ' + __k + ' is required');
      }
    }
