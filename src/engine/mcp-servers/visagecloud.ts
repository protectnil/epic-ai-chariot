...
export class VisageCloudMCPServer extends MCPAdapterBase {
  ...
  constructor(config: VisageCloudConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('VisageCloud: configuration object is required');
    }
    for (const __k of (['accessKey', 'secretKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('VisageCloud: ' + __k + ' is required');
      }
    }
