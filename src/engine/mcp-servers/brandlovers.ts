...
export class BrandLoversMCPServer extends MCPAdapterBase {
  ...
  private parseJsonArg(value: unknown, fieldName: string): unknown[] | ToolResult {
    if (!value) return [];
    try {
      const parsed: unknown = JSON.parse(value as string) as unknown;
