...
export class OpenBankingUkEventNotificationsOpenapiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenBankingUKEventNotificationsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Open Banking UK Event Notification API: configuration object is required');
    }
    for (const __k of (['bearerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Open Banking UK Event Notification API: ' + __k + ' is required');
      }
    }
