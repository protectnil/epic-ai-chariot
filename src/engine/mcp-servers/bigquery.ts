...
export class BigQueryMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BigQueryConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Google BigQuery: configuration object is required');
    }
    for (const __k of (['token', 'projectId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Google BigQuery: ' + __k + ' is required');
      }
    }
