...
export class AzureDevOpsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AzureDevOpsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Azure DevOps: configuration object is required');
    }
    for (const __k of (['organization', 'personalAccessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Azure DevOps: ' + __k + ' is required');
      }
    }
