...
export class OrbitLoveMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OrbitLoveConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Orbit Love: configuration object is required');
    }
    for (const __k of (['apiKey', 'workspaceSlug'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Orbit Love: ' + __k + ' is required');
      }
    }
