...
export class MicrocksMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MicrocksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Microcks: configuration object is required');
    }
    for (const __k of (['baseUrl', 'bearerToken'] as Array<keyof MicrocksConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Microcks: ' + __k + ' is required');
      }
    }
