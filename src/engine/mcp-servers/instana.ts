...
export class InstanaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: InstanaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('IBM Instana: configuration object is required');
    }
    for (const __k of (['baseUrl', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('IBM Instana: ' + __k + ' is required');
      }
    }
