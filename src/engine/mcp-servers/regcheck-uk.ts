...
export class RegCheckUKMCPServer extends MCPAdapterBase {
  ...
  constructor(config: RegCheckUKConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('RegCheck UK — Car Registration API: configuration object is required');
    }
    for (const __k of (['username', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('RegCheck UK — Car Registration API: ' + __k + ' is required');
      }
    }
