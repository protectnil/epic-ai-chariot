...
export class CircleCIMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CircleCIConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Circle CI: configuration object is required');
    }
    for (const __k of (['circleToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Circle CI: ' + __k + ' is required');
      }
    }
