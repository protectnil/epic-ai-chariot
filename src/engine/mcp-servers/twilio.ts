...
export class TwilioMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TwilioConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Twilio: configuration object is required');
    }
    for (const __k of (['accountSid', 'authToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Twilio: ' + __k + ' is required');
      }
    }
