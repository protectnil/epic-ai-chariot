...
export class CoralogixMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CoralogixConfig) {
    super();
    if (config === null) { throw new Error('CoralogixMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Coralogix: configuration object is required');
    }
    for (const __k of (['apiKey', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Coralogix: ' + __k + ' is required');
      }
    }
