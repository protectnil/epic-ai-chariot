...
export class SnowflakeMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SnowflakeConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Snowflake: configuration object is required');
    }
    for (const __k of (['account', 'token'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Snowflake: ' + __k + ' is required');
      }
    }
    this.config = config;
