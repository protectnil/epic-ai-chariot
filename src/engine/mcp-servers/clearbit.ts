...
export class ClearbitMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ClearbitConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Clearbit / HubSpot Breeze Intelligence: configuration object is required');
    }
    for (const __k of (['apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Clearbit / HubSpot Breeze Intelligence: ' + __k + ' is required');
      }
    }
