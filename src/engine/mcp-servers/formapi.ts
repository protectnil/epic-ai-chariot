...
export class FormAPIMCPServer extends MCPAdapterBase {
  ...
  constructor(config: FormAPIConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('formapi: configuration object is required');
    }
    for (const __k of (['apiTokenId', 'apiTokenSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('formapi: ' + __k + ' is required');
      }
    }
