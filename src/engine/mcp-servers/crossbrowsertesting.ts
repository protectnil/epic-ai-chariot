...
export class CrossBrowserTestingMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CrossBrowserTestingConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('CrossBrowserTesting Screenshot Comparisons: configuration object is required');
    }
    for (const __k of (['username', 'apiKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CrossBrowserTesting Screenshot Comparisons: ' + __k + ' is required');
      }
    }
