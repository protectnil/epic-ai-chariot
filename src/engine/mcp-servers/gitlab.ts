...
export class GitLabMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GitLabConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('GitLab: configuration object is required');
    }
    for (const __k of (['privateToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('GitLab: ' + __k + ' is required');
      }
    }
