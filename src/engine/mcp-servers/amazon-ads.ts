...
export class AmazonAdsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AmazonAdsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Amazon Ads: configuration object is required');
    }
    for (const __k of (['clientId', 'clientSecret', 'refreshToken', 'profileId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Amazon Ads: ' + __k + ' is required');
      }
    }
