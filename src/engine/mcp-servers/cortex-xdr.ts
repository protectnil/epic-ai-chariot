...
export class CortexXDRMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CortexXDRConfig) {
    super();
    if (config === null) { throw new Error('CortexXDRMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Palo Alto Cortex XDR: configuration object is required');
    }
    for (const __k of (['apiKey', 'apiKeyId', 'fqdn'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Palo Alto Cortex XDR: ' + __k + ' is required');
      }
    }
