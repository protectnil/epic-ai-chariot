...
export class PaloAltoMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PaloAltoConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Palo Alto Networks Cortex XDR: configuration object is required');
    }
    for (const __k of (['apiKey', 'apiKeyId', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Palo Alto Networks Cortex XDR: ' + __k + ' is required');
      }
    }
