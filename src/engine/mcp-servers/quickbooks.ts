...
export class QuickBooksMCPServer extends MCPAdapterBase {
  ...
  constructor(config: QuickBooksConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('QuickBooks Online: configuration object is required');
    }
    for (const __k of (['accessToken', 'realmId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('QuickBooks Online: ' + __k + ' is required');
      }
    }
  ...
  private async fetchJson(url: string, init?: RequestInit): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(url, {
      ...
      headers: {
        ...headers,
        'Accept': 'application/json', // Ensure Accept header is always set
      },
    });
    if (!response.ok) {
      return {
