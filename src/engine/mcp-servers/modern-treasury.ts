...
export class ModernTreasuryMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ModernTreasuryConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Modern Treasury: configuration object is required');
    }
    for (const __k of (['organizationId', 'apiKey'] as Array<keyof ModernTreasuryConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Modern Treasury: ' + __k + ' is required');
      }
    }
