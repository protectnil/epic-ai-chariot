...
export class RunscopeMCPServer extends MCPAdapterBase {
  ...
  private async request(method: string, path: string, body?: unknown): Promise<unknown> {
    ...
    return str.length > 10240 ? JSON.parse(str.slice(0, 10240) + '"') as unknown : data;
  }
