...
export class OpenBankingUkPaymentInitiationOpenapiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OpenBankingUKPaymentInitiationConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Open Banking UK Payment Initiation API: configuration object is required');
    }
    for (const __k of (['bearerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Open Banking UK Payment Initiation API: ' + __k + ' is required');
      }
    }
