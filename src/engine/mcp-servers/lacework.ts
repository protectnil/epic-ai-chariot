...
export class LaceworkMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LaceworkConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Lacework: configuration object is required');
    }
    for (const __k of (['keyId', 'secret'] as Array<keyof LaceworkConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Lacework: ' + __k + ' is required');
      }
    }
