...
export class GetgoGototrainingMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GoToTrainingConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('GoToTraining: configuration object is required');
    }
    for (const __k of (['accessToken', 'organizerKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('GoToTraining: ' + __k + ' is required');
      }
    }
