...
export class TelematicsSDKMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TelematicsSDKConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('TelematicsSDK: configuration object is required');
    }
    for (const __k of (['deviceToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('TelematicsSDK: ' + __k + ' is required');
      }
    }
