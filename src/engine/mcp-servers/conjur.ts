...
export class ConjurMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ConjurConfig) {
    super();
    if (config === null) { throw new Error('ConjurMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('CyberArk Conjur: configuration object is required');
    }
    for (const __k of (['account', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('CyberArk Conjur: ' + __k + ' is required');
      }
    }
