...
export class QualtricsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: QualtricsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Qualtrics: configuration object is required');
    }
    for (const __k of (['apiToken', 'datacenterId'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Qualtrics: ' + __k + ' is required');
      }
    }
