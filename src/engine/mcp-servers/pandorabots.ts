...
export class PandorabotsMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PandorabotsConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Pandorabots: configuration object is required');
    }
    for (const __k of (['appId', 'userKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Pandorabots: ' + __k + ' is required');
      }
    }
