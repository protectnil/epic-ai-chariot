...
export class AnsibleMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AnsibleConfig) {
    super();
    if (config === null) { throw new Error('AnsibleMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Ansible: configuration object is required');
    }
    for (const __k of (['token', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Ansible: ' + __k + ' is required');
      }
    }
