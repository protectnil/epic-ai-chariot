...
export class PostmarkMCPServer extends MCPAdapterBase {
  ...
  constructor(config: PostmarkConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Postmark: configuration object is required');
    }
    for (const __k of (['serverToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Postmark: ' + __k + ' is required');
      }
    }
