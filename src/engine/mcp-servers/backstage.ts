...
export class BackstageMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BackstageConfig) {
    super();
    if (config === null) { throw new Error('BackstageMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Backstage: configuration object is required');
    }
    for (const __k of (['token', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Backstage: ' + __k + ' is required');
      }
    }
