...
export class DatadogObservabilityMCPServer extends MCPAdapterBase {
  ...
  constructor(config: DatadogObservabilityConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('datadog-observability: configuration object is required');
    }
    for (const __k of (['apiKey', 'applicationKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('datadog-observability: ' + __k + ' is required');
      }
    }
