...
export class ExaVaultMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ExaVaultConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('exavault: configuration object is required');
    }
    for (const __k of (['apiKey', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('exavault: ' + __k + ' is required');
      }
    }
