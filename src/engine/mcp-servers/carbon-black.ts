...
export class CarbonBlackMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CarbonBlackConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Carbon Black Cloud: configuration object is required');
    }
    for (const __k of (['orgKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Carbon Black Cloud: ' + __k + ' is required');
      }
    }
