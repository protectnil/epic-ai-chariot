...
export class AircallMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AircallConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Aircall: configuration object is required');
    }
    for (const __k of (['apiId', 'apiToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Aircall: ' + __k + ' is required');
      }
    }
