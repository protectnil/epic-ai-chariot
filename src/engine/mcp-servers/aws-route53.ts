...
export class AWSRoute53MCPServer extends MCPAdapterBase {
  ...
  constructor(config: AWSRoute53Config) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('AWS Route 53: configuration object is required');
    }
    for (const __k of (['accessKeyId', 'secretAccessKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('AWS Route 53: ' + __k + ' is required');
      }
    }
...
<CreateHostedZoneRequest xmlns="https://route53.amazonaws.com/doc/2013-04-01/">
  ...
  private async changeResourceRecordSets(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    try { changes = JSON.parse(args.changes as string) as { Action: string; ResourceRecordSet: { Name: string; Type: string; TTL?: number; ResourceRecords?: { Value: string }[] } }[]; } catch { return { content: [{ type: 'text', text: 'changes must be valid JSON' }], isError: true }; }
    const id = this.normalizeZoneId(args.hosted_zone_id as string);
