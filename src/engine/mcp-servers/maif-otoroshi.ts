...
export class MaifOtoroshiMCPServer extends MCPAdapterBase {
  ...
  constructor(config: OtoroshiConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Otoroshi: configuration object is required');
    }
    for (const __k of (['baseUrl', 'clientId', 'clientSecret'] as Array<keyof OtoroshiConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Otoroshi: ' + __k + ' is required');
      }
    }
