...
export class GongMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GongConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Gong: configuration object is required');
    }
    for (const __k of (['accessKey', 'accessKeySecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Gong: ' + __k + ' is required');
      }
    }
