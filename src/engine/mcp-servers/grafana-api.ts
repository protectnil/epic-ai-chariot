...
export class GrafanaAPIMCPServer extends MCPAdapterBase {
  ...
  constructor(config: GrafanaAPIConfig) {
    super();
    if (config === null) { throw new Error('GrafanaAPIMCPServer: configuration object is required when provided'); }
    if (!config || typeof config !== 'object') {
      throw new Error('Grafana: configuration object is required');
    }
    for (const __k of (['serviceAccountToken', 'baseUrl'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Grafana: ' + __k + ' is required');
      }
    }
