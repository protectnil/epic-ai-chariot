...
export class BeanstreamMCPServer extends MCPAdapterBase {
  ...
  constructor(config: BeanstreamConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Beanstream Payments: configuration object is required');
    }
    for (const __k of (['merchantId', 'apiPasscode'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Beanstream Payments: ' + __k + ' is required');
      }
    }
