...
export class EuropeanaMCPServer extends MCPAdapterBase {
  ...
  constructor(config: EuropeanaConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('europeana: configuration object is required');
    }
    for (const __k of (['wskey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('europeana: ' + __k + ' is required');
      }
    }
