...
export class JFrogArtifactoryMCPServer extends MCPAdapterBase {
  ...
  constructor(config: JFrogArtifactoryConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('JFrog Artifactory: configuration object is required');
    }
    for (const __k of (['baseUrl', 'accessToken'] as Array<keyof JFrogArtifactoryConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('JFrog Artifactory: ' + __k + ' is required');
      }
    }
