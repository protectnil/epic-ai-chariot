...
export class LightspeedMCPServer extends MCPAdapterBase {
  ...
  constructor(config: LightspeedConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Lightspeed: configuration object is required');
    }
    for (const __k of (['accessToken', 'accountId'] as Array<keyof LightspeedConfig>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Lightspeed: ' + __k + ' is required');
      }
    }
