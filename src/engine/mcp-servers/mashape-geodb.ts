...
export class MashapeGeodbMCPServer extends MCPAdapterBase {
  ...
  constructor(config: MashapeGeodbConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('GeoDB Cities: configuration object is required');
    }
    if (config.rapidApiKey === undefined || config.rapidApiKey === null || config.rapidApiKey === '') {
      throw new Error('GeoDB Cities: rapidApiKey is required');
    }
