...
export class ServiceNowITSMMCPServer extends MCPAdapterBase {
  ...
  constructor(config: ServiceNowITSMConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('ServiceNow ITSM: configuration object is required');
    }
    for (const __k of (['instance'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('ServiceNow ITSM: ' + __k + ' is required');
      }
    }
