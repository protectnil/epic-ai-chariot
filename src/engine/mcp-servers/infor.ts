...
export class InforMCPServer extends MCPAdapterBase {
  ...
  constructor(config: InforConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Infor CloudSuite: configuration object is required');
    }
    for (const __k of (['baseUrl', 'bearerToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Infor CloudSuite: ' + __k + ' is required');
      }
    }
