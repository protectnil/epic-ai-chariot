...
export class TerraformRegistryMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TerraformRegistryConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Terraform: configuration object is required');
    }
    for (const __k of (['token', 'organization'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Terraform: ' + __k + ' is required');
      }
    }
