...
export class SegmentMCPServer extends MCPAdapterBase {
  ...
  constructor(config: SegmentConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Segment: configuration object is required');
    }
    for (const __k of (['writeKey'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('Segment: ' + __k + ' is required');
      }
    }
  ...
  private publicApiTokenChecked(): string {
    if (!this.publicApiToken) throw new Error('invariant: publicApiToken not set after requirePublicApiToken');
  ...
  private async publicApiGet(path: string, params?: URLSearchParams): Promise<ToolResult> {
    ...
    const response = await this.fetchWithRetry(`${this.publicApiBaseUrl}${path}${qs}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${this.publicApiTokenChecked()}`,
        'Content-Type': 'application/json',
      },
    });
