...
export class WooCommerceMCPServer extends MCPAdapterBase {
  ...
  constructor(config: WooCommerceConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('WooCommerce: configuration object is required');
    }
    for (const __k of (['storeUrl', 'consumerKey', 'consumerSecret'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('WooCommerce: ' + __k + ' is required');
      }
    }
