...
export class AzureMLMCPServer extends MCPAdapterBase {
  ...
  constructor(config: AzureMLConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Azure Machine Learning: configuration object is required');
    }
    for (const __k of (['subscriptionId', 'resourceGroupName', 'workspaceName', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Azure Machine Learning: ' + __k + ' is required');
      }
    }
