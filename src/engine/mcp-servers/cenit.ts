...
export class CenitMCPServer extends MCPAdapterBase {
  ...
  constructor(config: CenitConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('Cenit IO: configuration object is required');
    }
    for (const __k of (['accessKey', 'accessToken'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || (config[__k] as unknown) === '') {
        throw new Error('Cenit IO: ' + __k + ' is required');
      }
    }
