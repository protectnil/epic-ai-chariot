...
export class TheTradeDeskMCPServer extends MCPAdapterBase {
  ...
  constructor(config: TheTradeDeskConfig) {
    ...
    if (!config || typeof config !== 'object') {
      throw new Error('The Trade Desk: configuration object is required');
    }
    for (const __k of (['login', 'password'] as Array<keyof typeof config>)) {
      if (config[__k] === undefined || config[__k] === null || config[__k] === '') {
        throw new Error('The Trade Desk: ' + __k + ' is required');
      }
    }
