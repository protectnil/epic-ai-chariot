#!/usr/bin/env node
/**
 * Epic AI® Chariot — Tier 3 Eval (BM25 routing only)
 * Tests routing via MCP by calling chariot_query directly — does NOT invoke an LLM.
 * The eval sends queries through the ToolPreFilter and checks if the top-ranked
 * adapter is in the expected list. This validates BM25 ranking quality,
...
interface ChariotQueryResponse {
  matchedAdapters?: Array<{ id: string }>;
  adapters?: Array<{ id: string }>;
}

function parseChariotResponse(content: unknown): ChariotQueryResponse {
  if (!Array.isArray(content) || content.length === 0) return {};
  const first = content[0] as { type?: string; text?: string };
  if (typeof first.text !== 'string') return {};
  const parsed: unknown = JSON.parse(first.text);
  if (typeof parsed === 'object' && parsed !== null) {
    return parsed as ChariotQueryResponse;
  }
  return {};
}
...
async function main() {
  console.log('Epic AI® Chariot — Tier 3 Eval (BM25 routing)');
  console.log('═══════════════════════════════════════════\n');
  ...
  for (let i = 0; i < QUERIES.length; i++) {
    ...
    // discover:true exercises the full catalog index, not the (empty) set of
    // configured adapters this eval-process never adds via `chariot add`.
    const r1 = await client.callTool({ name: 'chariot_query', arguments: { query: q, discover: true } });
    const data1 = parseChariotResponse(r1.content);
    const adapters1: string[] = (data1.matchedAdapters ?? []).map(a => a.id);

    let found = expect.some(e => adapters1.includes(e));

    // Step 2: Ring 2 — get all adapter summaries and check
    if (!found) {
      const r2 = await client.callTool({ name: 'chariot_query', arguments: { query: q, detail: 'summary', discover: true } });
      const data2 = parseChariotResponse(r2.content);
      const adapters2: string[] = (data2.adapters ?? []).map(a => a.id);
