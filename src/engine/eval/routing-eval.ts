#!/usr/bin/env node
/**
 * Epic AI® Chariot — Routing Eval Suite
 ...
 *         appear in the top 8 results? Covers every adapter in the bundled catalog.
 *
...
// dist/engine/eval/routing-eval.js → 3 levels up to package root
...
function unpackCatalogPayload(payload: unknown): CatalogEntry[] {
  if (Array.isArray(payload)) return payload as CatalogEntry[];
  if (payload && typeof payload === 'object' && Array.isArray((payload as { catalog?: unknown }).catalog)) {
    return (payload as { catalog: CatalogEntry[] }).catalog;
  }
  return [];
}

function unpackRegistryPayload(payload: unknown): RegistryEntry[] {
  if (Array.isArray(payload)) return payload as RegistryEntry[];
  if (payload && typeof payload === 'object' && Array.isArray((payload as { registry?: unknown }).registry)) {
    return (payload as { registry: RegistryEntry[] }).registry;
  }
  return [];
}

function loadCatalog(): CatalogEntry[] {
  const bundleDir = resolvePublishedArtifactDir(PKG_ROOT, 'chariot-current.json', PKG_ROOT);
  const bundlePath = join(bundleDir, 'chariot-adapter-bundle.json');
  const raw = readFileSync(
    process.env.CHARIOT_ADAPTER_CATALOG_PATH ?? bundlePath,
    'utf-8',
  );
  return unpackCatalogPayload(JSON.parse(raw));
}
...
function loadRegistry(): RegistryEntry[] {
  ...
  const bundlePath = join(bundleDir, 'chariot-adapter-bundle.json');
  const raw = readFileSync(
    process.env.CHARIOT_MCP_REGISTRY_PATH ?? bundlePath,
    'utf-8',
  );
  return unpackRegistryPayload(JSON.parse(raw));
}
...
async function runTier1(filter: ToolPreFilter, queries: { query: string; expectedServer: string }[]): Promise<EvalResult[]> {
  return Promise.all(queries.map(async (q): Promise<EvalResult> => {
    const selected = await filter.select(q.query, { maxTools: 8, maxPerServer: 3 });
    const uniqueServers = [...new Set(selected.map(t => t.server))];
...
async function runTier2(filter: ToolPreFilter, corpus: { query: string; expectedServers: string[] }[]): Promise<EvalResult[]> {
  return Promise.all(corpus.map(async (q): Promise<EvalResult> => {
    const selected = await filter.select(q.query, { maxTools: 8, maxPerServer: 3 });
...
async function main(): Promise<void> {
  console.log('Epic AI® Chariot — Routing Eval Suite');
  console.log('════════════════════════════════════\n');
  ...
  const t2Hits = tier2Results.filter(r => r.hit).length;
  const t2Total = tier2Results.length;
  const t2Accuracy = t2Total === 0 ? '0.0' : ((t2Hits / t2Total) * 100).toFixed(1);

  console.log(`  Accuracy: ${t2Hits}/${t2Total} (${t2Accuracy}%) — informational only, never gates CI`);
  ...
  // ── Summary + per-tier exit ─────────────────────────────
  // Tier 1 is the regression gate (deterministic ranking of vendor-name
  // queries against the actual published catalog). It exits non-zero
  // when the score falls below CHARIOT_EVAL_TIER1_FLOOR (default 80).
  // Tier 2 is the catalog-vs-customer-expectation gap-tracker — its
  // expected vendors include mainstream names (Datadog, Stripe, GitHub,
  // PagerDuty, Salesforce, etc.) that may not be in the bundled catalog
  // at any given catalog-publication moment. It is reported but never
  // exits non-zero.
  console.log('\n════════════════════════════════════');
  console.log('ROUTING EVAL SUMMARY');
  console.log('════════════════════════════════════');
  console.log(`Tier 1 (generated, deterministic):  ${t1Accuracy}% (${t1Hits}/${t1Total})`);
  console.log(`Tier 2 (natural language, info):    ${t2Accuracy}% (${t2Hits}/${t2Total})`);
  console.log(`Total tools indexed:                ${tools.length}`);
  console.log(`Total queries:                      ${t1Total + t2Total}`);
  console.log(`Total time:                         ${t1Ms + t2Ms}ms`);

  const floorEnv = process.env.CHARIOT_EVAL_TIER1_FLOOR;
  const t1Floor = floorEnv ? Number(floorEnv) : 80;
  if (!Number.isFinite(t1Floor) || t1Floor < 0 || t1Floor > 100) {
    console.error(
      `\n✗ CHARIOT_EVAL_TIER1_FLOOR=${floorEnv} is not a valid percentage (0-100)`,
    );
    process.exit(2);
  }
  const t1AccuracyNum = (t1Hits / t1Total) * 100;
  if (t1AccuracyNum < t1Floor) {
    console.log(
      `\n✗ FAIL — Tier 1 accuracy ${t1Accuracy}% is below floor ${t1Floor}% ` +
        `(set via CHARIOT_EVAL_TIER1_FLOOR; default 80)`,
    );
    process.exit(1);
  }
  console.log(
    `\n✓ PASS — Tier 1 accuracy ${t1Accuracy}% ≥ floor ${t1Floor}% ` +
      `(Tier 2 ${t2Accuracy}% is informational; not a gate)`,
  );
}
