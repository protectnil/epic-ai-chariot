/**
 * @epicai/chariot — Generator Provider
...
export function createGeneratorLLM(config: GeneratorConfig): LLMFunction {
  ...
  switch (config.provider) {
    ...
    case 'digitalocean':
      return createDigitalOceanGenAILLM(config);
    case 'custom':
...
/**
 * DigitalOcean Serverless GenAI Inference — OpenAI-compatible chat
 * completions. Operator pins the model + the inference endpoint base
 * URL (e.g. https://inference.do-ai.run/v1) via DO_GENAI_BASE_URL.
 * Reuses the OpenAI LLM builder with a baseUrl override.
 */
function createDigitalOceanGenAILLM(config: GeneratorConfig): LLMFunction {
  if (!config.baseUrl) {
    throw new Error('DigitalOcean GenAI generator requires baseUrl (e.g. https://inference.do-ai.run/v1)');
  }
  if (!config.apiKey) {
    throw new Error('DigitalOcean GenAI generator requires an apiKey');
  }
  // DO's endpoint includes the /v1 path segment itself, so strip the
  // trailing /v1 from baseUrl before delegating — createOpenAILLM
  // appends /v1/chat/completions.
  const normalized = config.baseUrl.replace(/\/v1\/?$/, '');
  return createOpenAILLM({ ...config, baseUrl: normalized });
}
  ...
  return async (params: { messages: LLMMessage[]; tools?: LLMToolDefinition[] }): Promise<LLMResponse> => {
    ...
    try {
      ...
      return {
        ...
        toolCalls,
        // OWASP LLM09 Risk Communication: preserve granular OpenAI
        // finish_reason values (`content_filter`, `length`,
        // `tool_calls`, `stop`, plus refusal-style aliases) so the
        // orchestrator can derive a no-narrative reason when synthesis
        // returns empty content. Unknown / null values collapse to
        // `stop` so existing callers that only check the discriminator
        // keep working.
        finishReason:
          choice?.finish_reason === 'tool_calls' ? 'tool_calls' :
          choice?.finish_reason === 'length' ? 'length' :
          choice?.finish_reason === 'content_filter' ? 'content-filter' :
          choice?.finish_reason === 'refusal' ? 'refusal' :
          'stop',
      };
    } finally {
  ...
  return async (params: { messages: LLMMessage[]; tools?: LLMToolDefinition[] }): Promise<LLMResponse> => {
    ...
    try {
      ...
      return {
        content,
        toolCalls,
        // OWASP LLM09: preserve Anthropic stop_reason granularity.
        // 'max_tokens' → length truncation; 'refusal' (Sonnet 4.5+
        // safety-policy stop) → refusal; 'tool_use' → tool_calls;
        // 'end_turn' / 'stop_sequence' / unknown → stop.
        finishReason:
          data.stop_reason === 'tool_use' ? 'tool_calls' :
          data.stop_reason === 'max_tokens' ? 'length' :
          data.stop_reason === 'refusal' ? 'refusal' :
          'stop',
      };
    } finally {
      clearTimeout(timeout);
