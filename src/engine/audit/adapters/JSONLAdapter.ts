/**
 * @epicai/chariot — JSONL Audit Store Adapter
...
export class JSONLAdapter implements AuditStoreAdapter {
  ...
  async updateStatus(
    ...
    // Spec contract: only include when supplied; never write null/undefined.
    if (opts?.failureMode !== undefined) {
      payload.failureMode = opts.failureMode;
    }
    if (opts?.retryCount !== undefined) {
      payload.retryCount = opts.retryCount;
    }
    if (opts?.retryReasons !== undefined) {
      payload.retryReasons = opts.retryReasons;
    }
    if (opts?.outcome !== undefined) {
      payload.outcome = opts.outcome;
    }
    if (opts?.errorClass !== undefined) {
      payload.errorClass = opts.errorClass;
    }
    const update = JSON.stringify(payload, this.dateReplacer);
